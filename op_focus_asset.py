...
class ASSETBROWSER_OT_focus_asset(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if user_intent == 'COLLECTION':
            ...
            return focus_collection(context, self.focus_view, self)
        elif user_intent == 'ACTION':
            self.report({'INFO'}, f"Focus Action: {context.id.name}")
            return focus_action(context, self.focus_view)

        self.report({'ERROR'}, "Nothing to focus.")
        return {'CANCELLED'}


def focus_action(context, focus_view=True):
...
def focus_collection(context, focus_view=True, operator=None):
    colls_to_isolate = []
